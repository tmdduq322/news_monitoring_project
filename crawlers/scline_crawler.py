...
def scline_crw(wd, url, search):
    try:
        ...
        # 데이터 저장
        save_to_csv(main_temp, f'data/raw/16.사커라인/{today}/사커라인_{search}.csv')
        logging.info(f"data/raw/16.사커라인/{today}/사커라인_{search}.csv")
...
def scline_main_crw(searchs, start_date, end_date,stop_event):
    if not os.path.exists(f'data/raw/16.사커라인/{today}'):
        os.makedirs(f'data/raw/16.사커라인/{today}')
