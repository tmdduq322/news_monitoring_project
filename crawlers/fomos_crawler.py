...
def fomos_crw(wd, url, search):
    try:
        ...
        # 데이터 저장
        save_to_csv(main_temp, f'data/raw/18.포모스/{today}/포모스_{search}.csv')
        logging.info(f'data/raw/18.포모스/{today}/포모스_{search}.csv')
...
def fomos_main_crw(searchs, start_date, end_date, stop_event):
    if not os.path.exists(f'data/raw/18.포모스/{today}'):
        os.makedirs(f'data/raw/18.포모스/{today}')
